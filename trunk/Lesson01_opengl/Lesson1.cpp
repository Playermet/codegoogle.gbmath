...
GLvoid ReSizeGLScene(GLsizei width, GLsizei height)		// Resize And Initialize The GL Window
{
	...
	g_camera.setProjParams( gbmath::constan::PI/4.0f,(GLfloat)width/(GLfloat)height,0.1f,100.0f   );
...
int DrawGLScene(GLvoid)									// Here's Where We Do All The Drawing
{
	...
	glLoadIdentity();									// Reset The Current Modelview Matrix
	 ...
	 gbmath::drawer drawer;
 	 

	  //drawer.set_transform_matrix_proj( g_camera.getProjMatrix() );

	 mat44 world (1.0f);
			 static float curr_time = 0.0f;
			 curr_time = 0.001f * (float)::GetTickCount();
	// world.setRotationY(  curr_time  ) ;
	 drawer.set_transform_matrix_world(world);
										 
 
	 drawer.set_transform_matrix_view( g_camera.getViewMatrix() );
 
	g_camera.frameMove( 1.0f );


	drawer.set_color_green();
    __draw_cube();


	drawer.set_color_red();
	drawer.draw_line( vec3(0.0f , 0.0f, 0.0f ) , vec3(0.0f , 10.0f , 0.0f) );
